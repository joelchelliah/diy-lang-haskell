...
-- DiyLang programs.
-- You are not required to make any changes here as part of
-- the workshop.
...
-- and returns the result of the expression as
-- a string, along with the environment.
...
-- and returns the result of the last expression as
-- a string, along with the environment.
...
interpretFile fileName = do
  ...
  let parsedAsts        = parseMultiple content
      ...
      (ast', env')      = foldl' eval (DiySymbol "", Environment []) parsedAsts
